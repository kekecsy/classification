...
# from MySampler2 import ProgressiveLabelSampler
...
for index,value in enumerate(cluster_nodes):
    cluster_nodes_relations[str(index)] = go2index(value)
...
def prepare_dataloader(accelerator):
    ...
    train_sampler = MyDistributedSampler(trainset, num_replicas=accelerator.num_processes, rank=accelerator.process_index, shuffle=True, initial_ratio=0.5, drop_last=False)
    ...
    trainloader = MyDataLoader(trainset, batch_size=train_batch_size, collate_fn=collate_func, sampler=train_sampler, drop_last=True)
...
def train(model, optimizer, trainloader, validloader, accelerator: Accelerator, epoch=epoch, log_step=2, resume=None):
...
def main():
    ...
    accelerator = Accelerator(gradient_accumulation_steps=4, log_with="tensorboard", project_dir="ckpts3")
