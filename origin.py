...
from ProtTransModel import ClassConfig, T5EncoderCLSModel
...
def prepare_dataloader():
    ...
    for i in train_filelist:
        traindata_list.append(pd.read_csv(i, nrows=1e3))
    ...
    for i in test_filelist:
        validdata_list.append(pd.read_csv(i, nrows=1e3))
...
def prepare_model_and_optimizer(accelerator, validloader):
    ...
    optimizer = Adam([{'params': lora_layer_param, 'lr': 1e-5}])
    node_optimizer = Adam([{'params': node_param, 'lr': 1e-5}])


    return model, optimizer, node_optimizer
...
def train(model, optimizer, trainloader, validloader, accelerator: Accelerator, epoch=epoch, log_step=10, resume=None, node_optimizer=None):
    ...
    for ep in range(resume_epoch, epoch):
        ...
        for batch in active_dataloader:
            ...
            with accelerator.accumulate(model):
                ...
                params_before_nodeclassifier = {name: param.clone().detach() for name, param in model.named_parameters() \
                                                if 'node_classifier' in name}
                if not only_layer_metrics:
                    loss = layer_loss
                    ...
                    with torch.no_grad():
                        for name, param in model.named_parameters():
                            if 'layer_classifier' in name or 'lora_B' in name or 'lora_A' in name:
                                if not torch.allclose(params_before_layerclassifier[name], param):
                                    accelerator.print(f"{name} updated ")
                    # # 测试参数是否更新
                    with torch.no_grad():
                        for name, param in model.named_parameters():
                            if 'node_classifier' in name:
                                if not torch.allclose(params_before_nodeclassifier[name], param):
                                    accelerator.print(f"{name} updated ")
                else:
                    loss = layer_loss
...
def main():
    ...
    model, optimizer, node_optimizer = prepare_model_and_optimizer(accelerator, validloader)

    model, optimizer, trainloader, validloader = accelerator.prepare(model, optimizer, trainloader, validloader)

    node_optimizer = accelerator.prepare(node_optimizer)
    # train(model, optimizer, trainloader, validloader, accelerator, resume='./ckpts2/step_2400')

    train(model, optimizer, trainloader, validloader, accelerator, resume=None, node_optimizer=node_optimizer)
